...
inline std::optional<FailoverFirstTokenSample> ObserveTokenLatencyMetrics(
    ...
    absl::Time now,
    FailoverRecoveryDumper* failover_recovery_dumper = nullptr) {
  ...
  if (finished_on_prefill_instance) {
    ...
    if (sample.has_value()) {
      ...
      if (failover_recovery_dumper != nullptr) {
        failover_recovery_dumper->Dump(
            BuildFailoverRecoveryDumpRecord(*request, *sample, now));
      }
    }
    return sample;
  }
